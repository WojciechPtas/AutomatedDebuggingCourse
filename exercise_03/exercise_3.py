...
class Station:
    ...
    def repOK(self):
        ...
        for t in self.tracks:
            assert self is t.end or self is t.start
        #assert True
...
class TrainNetwork:
    ...
    def repOK(self):
        ...
        a =[t.train_id for t in self.trains] #Create list with trains' id
        assert len(set(a))==len(a) # lets check if every train id is unique         
        for t in self.tracks:
            t.repOK()
        ...
        for track in self.tracks:
            if track.train is not None:
                for track2 in self.tracks:
                    if track2 is not track:
                        if track2.end==track.start or track2.start==track.start or track2.end==track.end or track2.start==track.end:
                            assert track2.train is None
        #assert True
    ...
    def move_train(self, train: Train, from_track: Track, to_track: Track):
        ...
        assert to_track in self.tracks
        assert from_track in self.tracks
        assert train in self.trains
        assert from_track.end == to_track.start or from_track.end == to_track.start or from_track.start == to_track.start or from_track.start==to_track.end
        # No need to check if there are trains on a adjecent tracks, because it will be checked in repOK method
