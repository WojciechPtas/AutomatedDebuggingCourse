...
class DebuggerTests(BaseTests):
    ...
    def middle_tester(self, coverage=False) -> DependencyDebugger:
        ...
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_middle.MiddleTests.test_335'])
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_middle.MiddleTests.test_123'])
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_middle.MiddleTests.test_321'])
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_middle.MiddleTests.test_555'])
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_middle.MiddleTests.test_534'])
        with debugger.collect_fail(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_middle.MiddleTests.test_213'])
    ...
    def remove_html_markup_tester(self, coverage=False) -> DependencyDebugger:
        ...
        debugger = self.get_debugger(coverage=coverage)
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_remove_html_markup.RemoveHTMLMarkupTests.test_abc'])
        with debugger.collect_pass(Path('dump')):
            subprocess.run(
                ['python', '-m', 'unittest', 'test_remove_html_markup.RemoveHTMLMarkupTests.test_b_abc_b'])
        with debugger.collect_fail(Path('dump')):
            subprocess.run(
                ['python', '-m', 'unittest', 'test_remove_html_markup.RemoveHTMLMarkupTests.test_quoted_abc'])
    ...
    def sqrt_tester(self, coverage=False) -> DependencyDebugger:
        ...
        debugger = self.get_debugger(coverage=coverage)
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_sqrt.SqrtTests.test_9'])
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_sqrt.SqrtTests.test_2'])
        with debugger.collect_fail(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_sqrt.SqrtTests.test_neg_4'])
    ...
    def bf_tester(self, coverage=False) -> DependencyDebugger:
        ...
        debugger = self.get_debugger(coverage=coverage)
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_bf.BFTests.test_increase_ptr'])
        with debugger.collect_pass(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_bf.BFTests.test_increase_value'])
        with debugger.collect_fail(Path('dump')):
            subprocess.run(['python', '-m', 'unittest', 'test_bf.BFTests.test_decrease_value'])
