...
class ForwardTypeChecker(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> Optional[Type]:
        ...
        for x in node.body:
            self.visit(x)
        self.functions.put(node.name, (types,  self.current_return))
        self.current_return = None
    ...
    def visit_Assign(self, node: ast.Assign) -> Optional[Type]:
        ...
        #print(f"assign {ast.unparse(node.value)}")
        if ast.unparse(node.value) == "int":
            self.scope.put(target.id,Int)
        elif ast.unparse(node.value) == "str":
            self.scope.put(target.id,Str)
        else:
            self.scope.put(target.id, Anything)
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign) -> Optional[Type]:
        ...
        #print(ast.unparse(node.annotation))
        if ast.unparse(node.annotation) == "int":
            self.scope.put(node.target.id,Int)
            if Int != self.visit(node.value):
                raise TypeError
        elif ast.unparse(node.annotation) == "str":
            self.scope.put(node.target.id,Str)
            if Str != self.visit(node.value):
                raise TypeError
        else:
            self.scope.put(node.target.id, Anything)
            if Anything != self.visit(node.value):
                raise TypeError
        return None
    ...
    def visit_Return(self, node: ast.Return) -> Optional[Type]:
        ...
        # current_return type
        a= self.current_return
        b= self.visit(node.value)    
        if a == Anything:
            self.current_return=b
            return None
        else:
            if a!= b:
                raise TypeError
    ...
    def visit_Name(self, node: ast.Name) -> Optional[Type]:
        ...
        if self.scope.get(node.id) is None:
            raise TypeError
    ...
    def visit_BinOp(self, node: ast.BinOp) -> Optional[Type]:
        ...
        # Return the type of the expression
        if self.visit(node.left)==Int:
            if self.visit(node.right)==Str:
                raise TypeError
            else:
                return Int
        elif self.visit(node.left)==Str:
            if self.visit(node.right)==Int:
                raise TypeError
            else:
                return Str
        else:
    ...
    def visit_Call(self, node: ast.Call) -> Optional[Type]:
        ...
        for i in range(len(expected_types)):
            #print(f"expected: {expected_types[i]}, ecountederL {ast.unparse(node.args[i])}")
            #print(self.visit(node.args[i]))
            if expected_types[i]!=self.visit(node.args[i]):
                raise TypeError
...
def f(x: str) -> int:
    return x             
                ...
                ''',failing=True), 'test 8 failed'
    assert test('''
x: int = 2
y : str = 'va'
c = False                
                ''', failing=False), 'test 9 failed'
    #print("Succes!")
