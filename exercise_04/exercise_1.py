from ast import FunctionDef, NodeTransformer, fix_missing_locations, parse, expr, stmt, unparse, Return, Call
import inspect
...
class Transformer(NodeTransformer):
    ...
    def visit_FunctionDef(self, node: FunctionDef) -> FunctionDef:
        ...
        for arg in node.args.args:
            l.append(arg.arg + ' = {' + arg.arg + '} ')
        list_of_args=', '.join(l)
        to_inject="log(f\'{\"  \" * level}calling with "+ list_of_args + '\') '
        to_inject=parse(to_inject)
        node.body = [to_inject] + node.body
        node.args.args.append(parse('level: int = 0'))
        node.name=self.traced_name
        self.generic_visit(node)
        return node
    def visit_Return(self, node: Return) -> Return:
        stmt=parse('return returned('+unparse(node.value)+', level)')
        for i in stmt.body:
            self.generic_visit(i)
        return stmt
    def visit_Call(self, node: Call) -> Call:
        if unparse(node.func) == self.ori_name:
            node.func = parse(self.traced_name)
            node.args= node.args + [parse('level+1')]
        for i in node.args:
            self.generic_visit(i)
        return node
